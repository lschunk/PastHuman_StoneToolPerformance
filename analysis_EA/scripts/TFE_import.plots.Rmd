...
# filter the data for raw material 
flint <- filter(TFE_mean, Raw.material == "flint")
lydite <- filter(TFE_mean, Raw.material == "lydite")


p_flint <- ggplot(data = flint, 
      ...
      scale_x_continuous(breaks = unique(flint$strokes))  

print(p_flint)

p_lydite <- ggplot(data = lydite, 
      aes(x = strokes, y = Three_point.mean, colour = Edge.angle)) +
      geom_point(size = 2) +
      theme_classic() +
      geom_line(aes(group = ID)) +
      ...
      ylim(25, 110) + 
      scale_colour_manual(values = c("#D67236", "#B40F20")) + 
      scale_x_continuous(breaks = unique(lydite$strokes))  

print(p_lydite)

 # combine both plots 
    p_all <- p_lydite + p_flint + plot_layout(width = c(3/6, 3/6), guides = 'collect')  
