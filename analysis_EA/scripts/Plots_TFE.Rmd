...
```{r}
dir_in <- "analysis_EA/derived_data/"
...
```{r}
# selects only the data from the 'Three-point' method  
...
p_lydite <- ggplot(data = TFE_mean[grep("LYDIT", TFE_mean[["ID_cycle"]]), ], 
     ...
     # geom_text() +
      ...
      facet_wrap(Raw.material ~ Task + Contact.material, ncol = 2) +
...
p_lydite2 <- ggplot(data = TFE_mean[grep("LYDIT", TFE_mean[["ID_cycle"]]), ], 
             aes(x = strokes, y = Three_point.mean, colour = Edge.angle)) + 
             geom_point(size = 2) +
             geom_line(aes(group = ID)) +
...
p_flint2 <- ggplot(data = TFE_mean[grep("FLT", TFE_mean[["ID_cycle"]]), ], 
            aes(x = strokes, y = Three_point.mean, colour = Edge.angle)) + 
            geom_point(size = 2) +
            geom_line(aes(group = ID)) +
            facet_wrap(ID ~ Contact.material+Task, nrow = 3)+
