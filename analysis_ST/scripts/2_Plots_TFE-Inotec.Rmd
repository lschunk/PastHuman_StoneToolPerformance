...
good_data_cut <- filter(good_data, Task == "cutting")
...
# plots all depth points in one facet plot 
p3c <- ggplot(data = depth, aes(x = Angle, y = Depth.abs.depth, colour = Raw_material)) +
       geom_point() + labs(y = "Absolute depth (mm)") +
       facet_wrap(~Task, strip.position = "bottom") +
       # avoids overplotting of the labels (sample IDs)
       geom_text_repel(aes(label=Sample), size = 2, nudge_x = -0.4, 
...
# save to PDF
file_out <- paste0(file_path_sans_ext(info_in[["file"]]), "_P3c_depth_plot", ".pdf")
ggsave(filename = file_out, plot = p3c, path = dir_out, device = "pdf", 
       width = 180, units = "mm")
